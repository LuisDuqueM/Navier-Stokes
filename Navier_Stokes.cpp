...
void aplicar_fronteras(double* u, double* v, int Nx, int Ny, double velocidad_tapa) {
    ...
    // Condicion de velocidad cero en las paredes (pared izquierda y derecha)
    for(int y = 0; y < Ny; y++) {
        u[ IDX(0, y, Nx) ] = 0.0;
        v[ IDX(0, y, Nx) ] = 0.0;
        u[ IDX(Nx-1, y, Nx) ] = 0.0;
    ...
    // Condicion de velocidades en las tapas superior e inferior
    for(int x = 0; x < Nx; x++) {
        u[ IDX(x, 0, Nx) ] = 0.0;
        v[ IDX(x, 0, Nx) ] = 0.0;
        u[ IDX(x, Ny-1, Nx) ] = velocidad_tapa;
        v[ IDX(x, Ny-1, Nx) ] = 0.0; // Corregi la coma aqui (x, Ny-1)
    }
}
...
int main(){
    ...
    double dx = 0.0033;
    double dy = 0.0033;
    int pasos_totales = 10000;     // Para simular 1 segundo entero
    double nu = 0.0005;             // Viscosidad cinemática
